...
export class AudioCallGamePageComponent implements OnInit, OnDestroy {
  ...
  public showQuestions(
    ...
    this.audioCallGameService.getQuestionsList(
      group,
      fromWordList,
      page,
      authorization,
  ...
  private correct(word: Word): void {
    ...
    if (word.userWord?.difficulty !== undefined) {
      this.userWordService.putUserWord(id, obj).subscribe(() => {});
    } else {
      this.userWordService.postUserWord(id, obj).subscribe(() => {});
    }
  }
  ...
  public nextQuestion(): void {
    ...
    if (this.currentIndex < this.questionsList.length - 1) {
      ...
      this.percent =
        (correct.word.length / (correct.word.length + incorrect.word.length)) *
        100;
  ...
  public startFromBook(): void {
    if (this.audioCallGameService.fromBook) {
      this.showQuestions(
        this.audioCallGameService.dataFromBook.group,
        this.audioCallGameService.fromWordList,
        this.audioCallGameService.dataFromBook.page,
  ...
  public startFromWorldList(): void {
    if (this.audioCallGameService.fromWordList) {
      this.showQuestions(
        this.audioCallGameService.dataFromWordList.group,
        this.audioCallGameService.fromWordList,
        this.audioCallGameService.dataFromBook.page,
        this.isAuthenticated,
        this.userID
